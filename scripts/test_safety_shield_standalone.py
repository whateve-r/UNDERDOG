"""
Test Safety Shield - Prop Firm Compliance Validation (Standalone)

Validates PropFirmSafetyShield with realistic scenarios WITHOUT importing MT5 dependencies.
"""
...
# Import only SafetyShield (no MT5 dependencies)
import logging
...
# Import SafetyShield classes directly
exec(open(project_root / 'underdog' / 'execution' / 'safety_shield.py').read())
...
def print_result(scenario: str, is_safe: bool, action: dict):
    ...
    if 'current_dd' in action:
        print(f"  Current DD: {action['current_dd']*100:.2f}%")
    if 'limit' in action:
        print(f"  Limit: {action['limit']*100:.2f}%")
    if 'lot_size' in action:
        print(f"  Lot Size: {action['lot_size']:.3f}")
